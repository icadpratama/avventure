...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) . (empty($post_link) ? ' no_links' : '') ); ?>><?php
    ...
    // Post meta
    $avventure_post_meta = get_the_category_list( ' ' );
    if (isset($avventure_post_meta)) echo '<div class="post_featured_wrap"><div class="post_meta post_meta_in_featured">' . wp_kses_data($avventure_post_meta) . '</div>';
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('medium'), 'blogger-modern')
														), 'blogger-modern')
								);
    if (isset($avventure_post_meta)) echo '</div>';
