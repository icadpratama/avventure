...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['actions'] as $item) {
		...
		?><div class="sc_action_item sc_action_item_event<?php 
		...
		?>>
			...
			if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('full'), 'action-event'));
				if (!empty($item['image'])) {
					$attr = trx_addons_getimagesize($item['image']);
					?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e("Action's image", 'avventure'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_action_item_icon sc_action_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($svg)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'avventure'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
			...
			if (!empty($item["trx_sc_action_event_date"])) {
                ?><div class="sc_action_item_event_date"><?php echo esc_html($item["trx_sc_action_event_date"]); ?></div><?php
            }
            ...
            if (!empty($item['link']) && !empty($item['link_text']) && !empty($item['info'])) { ?><div class="sc_action_item_button_and_info"><?php }
                if (!empty($item['info'])) {
                ...
                if (!empty($item['link']) && !empty($item['link_text'])) {
                    ?><a href="<?php echo esc_url($item['link']); ?>" class="<?php
                        echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_action_item_link sc_button sc_button_size_default', 'sc_action', $args, $item));
                    ?>"<?php
            ...
            if (!empty($item['link']) && !empty($item['link_text']) && !empty($item['info'])) { ?></div><?php }
            if (!empty($item["trx_sc_action_countdown_date"]) || !empty($item["trx_sc_action_countdown_time"])) {
                ?><div class="sc_action_item_countdown"><?php
                    echo do_shortcode('[trx_sc_countdown "align"="left" date="'.$item["trx_sc_action_countdown_date"].'" time="'.$item["trx_sc_action_countdown_time"].'"]');
                ?></div><?php
            }
			if (!empty($item['bg_image']) || !empty($item['bg_color'])) {
				?></div><!-- /.sc_action_inner --><?php
				if (!empty($item['link']) && empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php
