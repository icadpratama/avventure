...
if ( has_post_thumbnail() ) {
	$image = trx_addons_get_attachment_url( 
											...
											apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('full'), 'services-hover')
											);
}
...
?>>
	<div class="sc_services_item_header <?php echo esc_attr($args['hide_bg_image']==1 ? ' without_image' : ''); ?>"<?php if ($args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		<div class="sc_services_item_header_inner">
			...
			if ($args['featured'] != 'none') {
				if ($args['featured']=='icon' && !empty($meta['icon'])) {
					...
					?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'avventure'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
					echo !empty($link) 
						? '</a>' 
						: '</span>';
		...
		?></div><?php
		if (!empty($link)) {
            ?><div class="sc_services_item_button"><a class="sc_button sc_button_default sc_button_size_normal sc_button_icon_right" href="<?php echo esc_url($link); ?>">
                <span class="sc_button_text"><span class="sc_button_title"><?php echo esc_html($args['more_text']); ?></span></span>
            </a></div><?php
		}
	?></div><?php
