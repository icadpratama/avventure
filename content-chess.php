...
if ( is_array( $avventure_template_args ) ) {
	$avventure_columns    = empty( $avventure_template_args['columns'] ) ? 1 : max( 1, min( 3, $avventure_template_args['columns'] ) );
	...
	$avventure_columns    = empty( $avventure_blog_style[1] ) ? 1 : max( 1, min( 3, $avventure_blog_style[1] ) );
}
...
?><article id="post-<?php the_ID(); ?>" 
									...
									post_class(
										'post_item'
										. ' post_layout_chess'
										. ' post_layout_chess_' . esc_attr( $avventure_columns )
	...
	<?php
	// Add anchor
	if ( 1 == $avventure_columns && ! is_array( $avventure_template_args ) && shortcode_exists( 'trx_sc_anchor' ) ) {
		echo do_shortcode( '[trx_sc_anchor id="post_' . esc_attr( get_the_ID() ) . '" title="' . esc_attr( get_the_title() ) . '" icon="' . esc_attr( avventure_get_post_icon() ) . '"]' );
	}
	...
	avventure_show_post_featured(
		array(
			'class'         => 1 == $avventure_columns && ! is_array( $avventure_template_args ) ? 'avventure-full-height' : '',
			'singular'      => false,
			...
			'thumb_size'    => avventure_get_thumb_size(
				strpos( avventure_get_theme_option( 'body_style' ), 'full' ) !== false
										? ( 1 < $avventure_columns ? 'huge' : 'original' )
										: ( 2 < $avventure_columns ? 'big' : 'huge' )
			),
		)
	);
	...
	<div class="post_inner"><div class="post_inner_content"><div class="post_header entry-header">
		...
		if ( empty( $avventure_template_args['no_links'] ) ) {
			...
			$avventure_post_meta  = empty( $avventure_components ) || in_array( $avventure_hover, array( 'border', 'pull', 'slide', 'fade' ) )
										? ''
			...
			avventure_show_layout( $avventure_post_meta );
			?>
		</div><!-- .entry-header -->
