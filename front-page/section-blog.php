<div class="front_page_section front_page_section_blog<?php
			$avventure_scheme = avventure_get_theme_option( 'front_page_blog_scheme' );
if ( ! avventure_is_inherit( $avventure_scheme ) ) {
	echo ' scheme_' . esc_attr( $avventure_scheme );
}
			echo ' front_page_section_paddings_' . esc_attr( avventure_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$avventure_css      = '';
		$avventure_bg_image = avventure_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$avventure_anchor_icon = avventure_get_theme_option( 'front_page_blog_anchor_icon' );
	$avventure_anchor_text = avventure_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( avventure_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$avventure_css      = '';
			$avventure_bg_mask  = avventure_get_theme_option( 'front_page_blog_bg_mask' );
			$avventure_bg_color_type = avventure_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $avventure_bg_color_type ) {
				$avventure_bg_color = avventure_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$avventure_caption = avventure_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $avventure_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $avventure_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $avventure_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$avventure_description = avventure_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $avventure_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $avventure_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $avventure_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! avventure_exists_trx_addons() ) {
					...
					avventure_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
