...
if ( ! function_exists( 'avventure_customizer_get_css' ) ) {

	function avventure_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = avventure_customizer_add_theme_fonts( $fonts );

			$rez          = array();
			$rez['fonts'] = <<<EOT

/*Space Mono*/
.trx_addons_scroll_to_top.trx_addons_icon-up, .trx_addons_scroll_to_top.trx_addons_icon-up:after,
html .minimal-dark .esg-navigationbutton,
...
span.amount, .woocommerce .widget_price_filter .price_slider_amount span,
.sc_services.sc_services_list .sc_services_item_title,
.elementor-accordion .elementor-tab-title,
.video-with-stamp .video-text,
.author_title, .comments_list_wrap .comment_posted, .comments_list_wrap .comment_reply,
...
.post_meta_single .post_meta_item.post_tags a,
...
.sc_skills_pie.sc_skills_compact_off .sc_skills_total, .sc_skills_pie.sc_skills_compact_off .sc_skills_item_title,
.trx_addons_dropcap{
...
.accent-text-element,
.style-for-descr .sc_title.sc_title_default .sc_item_descr, .style-text-descr,
.sc_action_default .sc_action_item_description,
...
EOT;
			$rez          = apply_filters( 'avventure_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = avventure_customizer_add_theme_vars( $vars );
			extract( $vars );
...
EOT;
			$rez          = apply_filters( 'avventure_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( avventure_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = avventure_get_scheme_colors( $s );
					}
...
.elementor-text-editor table th{
	border-color: {$colors['bg_color_07']} !important;
}
.elementor-text-editor table th:first-child{
	border-left-color: {$colors['extra_bg_color']} !important;
}
.elementor-text-editor table th:last-child{
	border-right-color: {$colors['extra_bg_color']} !important;
}
.elementor-text-editor table td,
.elementor-text-editor table td:last-child{
	border-color: {$colors['bd_color']} !important;
}
table th, .comment_text table tbody tr:first-child th {
	color: {$colors['extra_dark']};
	background-color: {$colors['extra_bg_color']};
}
...
.top_panel_custom_header:before,
.top_panel_custom_header-home:before {
	background-color: {$colors['bg_color']}
}
...
.sc_services_item  .more-link:hover,
...
html .minimal-light .esg-navigationbutton:hover,
button:hover,
button:focus,
...
.esg-grid .mainul li.eg-item-skin-blog-wrapper {
	background-color: {$colors['bg_color']};
}
...
.sc_layouts_menu_nav > li ul [class*="current-menu-"] > a .sc_layouts_menu_item_description,
...
.avventure_tabs .avventure_tabs_titles li a:hover {
	color: {$colors['inverse_text']};
	background-color: {$colors['text_link2']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.avventure_tabs .avventure_tabs_titles li.ui-state-active a {
	color: {$colors['inverse_text']};
	background-color: {$colors['text_link2']};
}
...
.sheme_self.sidebar .avventure_tabs .avventure_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .avventure_tabs .avventure_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['alter_link']};
}
...
.sheme_self.sidebar .avventure_tabs .avventure_tabs_titles li.ui-state-active a {
...
.mejs-container .mejs-controls .mejs-button.mejs-playpause-button > button:hover,
.mejs-container .mejs-controls .mejs-button.mejs-playpause-button > button:focus {
	color: {$colors['text_link']};
	background-color: {$colors['text_dark']} !important;
}
...
.scheme_self.sidebar .sidebar_inner aside{
	background-color: {$colors['alter_bg_color']};
}
...
EOT;
					...
					$rez            = apply_filters(
						'avventure_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'avventure_filter_prepare_css', $css_str, $remove_spaces );
	}
}
