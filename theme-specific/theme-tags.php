...
if ( ! function_exists( 'avventure_need_page_title' ) ) {
	function avventure_need_page_title() {
		return ! is_front_page() && apply_filters( 'avventure_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'avventure_show_layout' ) ) {
	function avventure_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'avventure_get_logo_image' ) ) {
	function avventure_get_logo_image( $type = '' ) {
		$logo_image = '';
		if ( avventure_is_on( avventure_get_theme_option( 'logo_retina_enabled' ) ) && avventure_get_retina_multiplier( 2 ) > 1 ) {
			$logo_image = avventure_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' );
		}
		if ( empty( $logo_image ) ) {
			if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
				...
				$logo_image = avventure_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
			}
		}
		return avventure_remove_protocol_from_url( $logo_image, false );
	}
}

// Return header video (if set)
if ( ! function_exists( 'avventure_get_header_video' ) ) {
	function avventure_get_header_video() {
		$video = '';
		if ( apply_filters( 'avventure_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( avventure_check_theme_option( 'header_video' ) ) {
				$video = avventure_get_theme_option( 'header_video' );
				if ( (int) $video > 0 ) {
...
if ( ! function_exists( 'avventure_show_post_meta' ) ) {
	function avventure_show_post_meta( $args = array() ) {
		if ( is_single() && avventure_is_off( avventure_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ', ' ) : apply_filters( 'avventure_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						?>
						<span class="post_meta_item post_categories"><?php avventure_show_layout( $cats ); ?></span>
						<?php
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'avventure_filter_get_post_date', avventure_get_date() );
					if ( ! empty( $dt ) ) {
						?>
						...
						>
							<?php if ( ! is_singular() ) { ?>
								<a href="<?php echo esc_url( get_permalink() ); ?>"><?php } ?><?php echo wp_kses_data( $dt ); ?><?php if ( ! is_singular() ) { ?></a>
							<?php } ?>
						</span>
						<?php
					}
					...
					// Post counters
					$output = avventure_get_post_counters( $args['counters'] );
					if ( '' != $output ) {
						avventure_show_layout( $output );
					} elseif ( ! avventure_exists_trx_addons() ) {
						if ( ! is_singular() || have_comments() || comments_open() ) {
							...
							?>
								<a href="<?php echo esc_url( get_comments_link() ); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><span class="post_counters_number">
									...
									<?php
									echo esc_html( _n( 'Comment', 'Comments', $post_comments, 'avventure' ) );
									?>
									</span>
								</a>
							<?php
					...
					// Socials share
					avventure_show_share_links(
						array(
							'type'    => 'drop',
							'caption' => esc_html__( 'Share', 'avventure' ),
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'avventure' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );
				} else {
					// Custom counter
					do_action( 'avventure_action_show_post_counter', $comp, get_the_ID() );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'avventure_show_post_featured' ) ) {
	function avventure_show_post_featured( $args = array() ) {
		$args = array_merge(
			array(
				'hover'         => avventure_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: avventure_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? avventure_get_no_image() : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = avventure_get_post_image();
				if ( ! empty( $image ) ) {
					$image = avventure_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . avventure_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}

		 // Post meta
        if ( $args['show_categories'] ) $avventure_post_meta = get_the_category_list( ' ' );
		...
		if ( $args['singular'] ) {
			...
			if ( is_attachment() ) {
                ...
                <?php if (isset($avventure_post_meta)) { echo '<div class="post_meta post_meta_in_featured">' . wp_kses_data($avventure_post_meta) . '</div>'; } ?>

				<?php
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						get_the_ID(), $thumb_size, false,
						avventure_is_on( avventure_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( avventure_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				if (isset($avventure_post_meta)) { echo '<div class="post_meta post_meta_in_featured">' . wp_kses_data($avventure_post_meta) . '</div>'; }

				if ( has_post_thumbnail() && $args['seo'] ) {
					$avventure_attr = avventure_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $avventure_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $avventure_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
                ...
                if (!in_array( $post_format, array( 'audio', 'quote' ) )) {
                    if (isset($avventure_post_meta)) { echo '<div class="post_meta post_meta_in_featured">' . wp_kses_data($avventure_post_meta) . '</div>'; }
				}
				...
				if ( $args['thumb_bg'] ) {
					...
					if ( ! in_array( $post_format, array( 'audio', 'video' ) ) ) {
						avventure_hovers_add_icons( $args['hover'], array( 'no_links' => $args['no_links'] ) );
					}
				} elseif ( $has_thumb ) {
					the_post_thumbnail(
						$thumb_size, array(
					...
					$output = avventure_get_slider_layout(
						array(
				...
				if ( ! $args['thumb_only'] && ( 'video' == $post_format || 'audio' == $post_format ) ) {
					$post_content = avventure_get_post_content();
					// Put video under the thumb
					if ( 'video' == $post_format ) {
						$video = avventure_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = avventure_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = avventure_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = avventure_make_video_autoplay( $video );
								?>
						...
						$audio = avventure_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = avventure_get_post_iframe( $post_content, false );
						}
						if ( empty( $audio ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', $post_content );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = avventure_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								$media = urldecode( avventure_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								$media_author = '';
								$media_title  = '';
								...
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php avventure_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php avventure_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								avventure_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				avventure_show_layout( $post_info );
...
if ( ! function_exists( 'avventure_get_no_image' ) ) {
	function avventure_get_no_image( $no_image = '' ) {
		static $img = '';
		if ( empty( $img ) ) {
			$img = avventure_get_theme_option( 'no_image' );
			if ( empty( $img ) ) {
				$img = avventure_get_file_url( 'images/no-image.jpg' );
			}
		}
		if ( ! empty( $img ) ) {
			$no_image = $img;
...
if ( ! function_exists( 'avventure_add_bg_in_post_nav' ) ) {
	function avventure_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), avventure_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( avventure_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), avventure_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( avventure_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'avventure-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'avventure_show_related_posts' ) ) {
	function avventure_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : avventure_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			<section class="related_wrap">
				...
				if ( ! empty( $title ) ) {
					...
					esc_html_e( 'You May Also Like', 'avventure' );
				}
				?>
				</h3>
				<div class="columns_wrap posts_container columns_padding_bottom">
					...
					while ( $query->have_posts() ) {
						...
						<div class="column-1_<?php echo intval( max( 1, min( 4, $args['columns'] ) ) ); ?>">
							<?php
							get_template_part( apply_filters( 'avventure_filter_get_template_part', 'templates/related-posts', $style ), $style );
							?>
						</div>
						<?php
					}
...
if ( ! function_exists( 'avventure_show_portfolio_posts' ) ) {
	function avventure_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? avventure_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = avventure_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = avventure_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$avventure_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the avventure_ajax_get_posts action
if ( ! function_exists( 'avventure_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_avventure_ajax_get_posts', 'avventure_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_avventure_ajax_get_posts', 'avventure_ajax_get_posts_callback' );
	function avventure_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'avventure' );
		}
...
// Show pagination
if ( ! function_exists( 'avventure_show_pagination' ) ) {
	function avventure_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = avventure_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'avventure' ),
					'next_text'          => esc_html__( '>', 'avventure' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'avventure' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'avventure' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'avventure' ) : esc_html__( 'Newest posts', 'avventure' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'avventure' ) : esc_html__( 'Older posts', 'avventure' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'avventure_single_comments_field' ) ) {
	function avventure_single_comments_field( $args ) {
